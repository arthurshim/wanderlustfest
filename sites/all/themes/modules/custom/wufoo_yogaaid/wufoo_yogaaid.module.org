...
function wufoo_yogaaid_menu() {
  $items['wufoo_webhook'] = array(
    'title' => "Callback for wufoo",
    'description' => "This URL is called by wufoo when we add a web hook",
    'page callback' => 'wufoo_yogaaid_webhook_callback',
    'access arguments' => array('access content'),
    'type' => MENU_CALLBACK,
  );
  $items['wufoo_redirect'] = array(
    'title' => "Redirect for wufoo",
    'description' => "This URL will hold and redirect",
    'page callback' => 'wufoo_yogaaid_webhook_redirect',
...
function wufoo_yogaaid_webhook_redirect($entityId) {
  /*EntityId of the form that was created*/

  $varname = 'wufoo_'.$entityId;
	$url = NULL;
	$error = FALSE;
	$count = 0;
  ...
  */
    ...
    $url = db_result(db_query("SELECT value FROM {variable} WHERE name='%s'",$varname));
		if (empty($url)) {
			$count++;
      ...
      $url = unserialize($url);
			...
			if ($url == 'Error') {
				...
				$varname = 'wufoo_'.$entityId.'_msg';
				$url = db_result(db_query("SELECT value FROM {variable} WHERE name='%s'",$varname));
			}
    }
  }
	file_put_contents('sites/default/files/webredirect.txt', 
		$varname
		.print_r($url, true)
		."count $count"
		."error $error"
	...
	elseif($error) {
		return t("There was an error registering your account with YogaAid.")."</br>".$url;
	}
	else {
		return t("We are having some technical difficulties, please try again in a short while");
	}
}

function wufoo_yogaaid_webhook_callback() {
...
//  $_POST['Field4'] = strtoupper($_POST['Field4']);
  ...
  file_put_contents('sites/default/files/webhook.txt',
    print_r($_POST,true).
  ...
  if ($res && $res->URL && !empty($res->URL)) {
    ...
    variable_set('wufoo_'.$_POST['EntryId'],$res->URL);
  }
  elseif ($res && $res->Status == 'Error')  {
		variable_set('wufoo_'.$_POST['EntryId'], $res->Status);
		variable_set('wufoo_'.$_POST['EntryId'].'_msg', $res->Message);
  }
  return 'hello';
}
